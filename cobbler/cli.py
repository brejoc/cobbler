...
class CobblerCLI:
    ...
    def __init__(self):
        ...
        self.remote = Server(self.url_cobbler_api)
    ...
    def check_setup(self):
        ...
        s = Server(self.url_cobbler_xmlrpc)
        try:
            s.ping()
        ...
        s = Server(self.url_cobbler_api)
        try:
            s.ping()
        except:
            print("httpd does not appear to be running and proxying cobbler, or SELinux is in the way. Original traceback:", file=sys.stderr)
            traceback.print_exc()
            sys.exit(411)
    ...
    def object_command(self, object_type, object_action):
        ...
        if object_action == "report":
            ...
            if object_action in ["add", "edit", "copy", "rename", "remove"]:
                ...
                except XmlRpcFault as xmlRpcFault:
                    (err) = xmlRpcFault
